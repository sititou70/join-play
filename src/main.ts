...
import { Value, Table, QueryResult } from "./types";
...
const innerJoin = (t1: Table, t2: Table): QueryResult => {
  let res: QueryResult = [];

  for (const val1 of t1) {
    for (const val2 of t2) {
      if (eq(val1, val2)) {
        res.push({ t1_val: val1, t2_val: val2 });
      }
    }
  }
...
const leftOuterJoin = (t1: Table, t2: Table): QueryResult => {
  let res: QueryResult = [];
  ...
  for (const val1 of t1) {
    ...
    if (!selected) res.push({ t1_val: val1, t2_val: null });
  }

  return res;
};

const fullOuterJoin = (t1: Table, t2: Table): QueryResult => {
  let res: QueryResult = [];
  ...
  for (const [idx1, val1] of t1.entries())
    if (!selectedT1Val.has(idx1)) res.push({ t1_val: val1, t2_val: null });
  for (const [idx2, val2] of t2.entries())
    if (!selectedT2Val.has(idx2)) res.push({ t1_val: null, t2_val: val2 });
...
const resetTable = (tableName: string, vals: Value[]): string => {
  const serializedVals = vals.map((row) => `(${row})`).join(",");
  return (
...
const innerJoinNaive = async (t1: Table, t2: Table): Promise<QueryResult> => {
